...
public:
	static cv::Mat generateFacade(int width, int height, int thickness, int default_NF, int default_NC, const std::vector<float>& params, const cv::Scalar& bg_color, const cv::Scalar& fg_color);
	static void decodeParams(float width, float height, int default_NF, int default_NC, const std::vector<float>& params, std::vector<float>& decoded_params);
